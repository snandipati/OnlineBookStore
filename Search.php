...
Search: <input type="text" id="term" name="term" />  <!-- option to enter the search pattern-->
...
$term = isset($_POST['term'])?$_POST['term']:null;
 ...
 where (p.productname like '%$term%' or p.productdescription like '%$term%' or c.categorytype like '%$term%' or m.manufacturername like '%$term%') and p.productid = p.productid and p.categoryid = c.categoryid and p.manufacturerid = m.manufacturerid order by p.productid");
